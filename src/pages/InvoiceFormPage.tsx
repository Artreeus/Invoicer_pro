    ...
    <div className="p-4 lg:p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">{isEditing ? 'Edit Invoice' : 'New Invoice'}</h1>
            <p className="text-sm text-gray-500">{invoiceNumber}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {isEditing && id && (
            <button onClick={() => navigate(`/invoices/${id}/preview`)} className="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
              <Eye size={16} /> Preview
            </button>
          )}
          <button onClick={() => handleSave('draft')} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition-colors">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={addItem} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-teal-600 hover:bg-teal-50 rounded-lg transition-colors">
              <Plus size={14} /> Add Item
            </button>
          </div>

          <div className="space-y-3">
            <div className="hidden lg:grid grid-cols-12 gap-2 px-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">
              ...
              <div key={item.id} className="relative bg-gray-50 rounded-lg p-3">
                <div className="lg:grid lg:grid-cols-12 gap-2 space-y-2 lg:space-y-0 items-start">
                  <div className="col-span-3 relative">
                    <div className="lg:hidden text-xs font-medium text-gray-500 mb-1">Item Name</div>
                    <input
                      className="w-full px-2.5 py-1.5 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                    ...
                    {itemSuggestions.show && itemSuggestions.index === index && libraryItems.filter(li => li.name.toLowerCase().includes(item.item_name.toLowerCase())).length > 0 && (
                      <div className="absolute z-10 mt-1 w-full bg-white rounded-lg shadow-lg border border-gray-100 py-1 max-h-40 overflow-y-auto">
                        {libraryItems.filter(li => li.name.toLowerCase().includes(item.item_name.toLowerCase())).map(li => (
                          <button key={li.id} onMouseDown={() => selectLibraryItem(index, li)} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 text-gray-700">
                            <div className="font-medium">{li.name}</div>
                            <div className="text-xs text-gray-400">৳{li.default_unit_price}</div>
                          </button>
                        ))}
                      </div>
                    )}
                    <input
                      className="w-full px-2.5 py-1 border-0 text-xs text-gray-400 focus:outline-none mt-1"
                  ...
                  <div className="col-span-1">
                    <div className="lg:hidden text-xs font-medium text-gray-500 mb-1">Quantity</div>
                    <input
                      type="number"
                      className="w-full px-2.5 py-1.5 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                      value={item.quantity}
                  ...
                  <div className="col-span-2">
                    <div className="lg:hidden text-xs font-medium text-gray-500 mb-1">Unit Price</div>
                    <input
                      type="number"
                      className="w-full px-2.5 py-1.5 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                  ...
                  <div className="col-span-2">
                    <div className="lg:hidden text-xs font-medium text-gray-500 mb-1">Discount</div>
                    <div className="flex gap-1">
                      <select
                        className="w-16 px-1 py-1.5 border border-gray-200 rounded-md text-xs focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                      ...
                      <input
                        type="number"
                        className="flex-1 px-2 py-1.5 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                  ...
                  <div className="col-span-1">
                    <div className="lg:hidden text-xs font-medium text-gray-500 mb-1">VAT %</div>
                    <select
                      className="w-full px-1 py-1.5 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                      value={item.vat_rate}
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          <h2 className="text-sm font-semibold text-gray-800 mb-4">Footer & Additional Info</h2>
